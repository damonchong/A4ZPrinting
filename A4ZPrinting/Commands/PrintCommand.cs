...
namespace A4ZPrinting
{
  public class PrintCommand : ZPLCommands
  {
    ...
    public PrintCommand(IZebraPrinter p)
    {
      ...
      {
        throw new NotSupportedException("The printer setup is not supported!");
      }
    }
    ...
    public byte[] ClearPrinter()
    {
      ...
      //_stringCounter = 0;
      if (printerModel.Label.Perforated)
      {
    ...
    public byte[] GeneratePrintCommands()
    {
      ...
      ILabel label = printerModel.Label;
      ...
      for (int i = 0; i < size; i++)
      {
        ...
        if (t is TextField)
        {
          ...
          {
            nextAsciiBytes = PrintCommand.TextWrite(left, top, t.Orientation, ((TextField)t).DefaultFont, height, t.Contents);
          }
          else if(holderTemplate is LeftJustifiedHolder)
          {
            ...
            nextAsciiBytes = PrintCommand.AlignWrite(left, top, t.Orientation, ((TextField)t).DefaultFont, height, width,
        ...
        else if(t is DataMatrixField)
        {
          ...
          nextAsciiBytes = PrintCommand.DataMatrixWrite(left, top, t.Orientation, ((DataMatrixField)t).DotsPerMatrixCell, t.Contents);          
        }
        else if(t is Code128Field)
        {
          nextAsciiBytes = PrintCommand.Code128Write(left, top, height, t.Orientation, ((Code128Field)t).NarrowBarModuleWidth, true, t.Contents);
        }
        asciiBytes = Utilities.Combine(asciiBytes, nextAsciiBytes);
      }

      nextAsciiBytes = PrintCommand.PrintBuffer();
      asciiBytes = Utilities.Combine(asciiBytes, nextAsciiBytes);
      return asciiBytes;
    }
